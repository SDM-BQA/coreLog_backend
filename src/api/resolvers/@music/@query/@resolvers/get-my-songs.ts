...
export const get_my_songs = async (_parent: any, args: { filter?: SongFilter }, ctx: any) => {
    try {
        const user = await get_auth_user(ctx.req);
        const filter = args.filter ?? {};

        const result = await quick_list(song_model, {
            base_query: { user_id: user._id },
            search: filter.search,
            search_fields: ["title", "artist", "album_name"],
            match: {
        ...
        return { songs: [], total_count: 0, current_page: 1, per_page: 10, page_count: 1, has_next_page: false };
    }
};
