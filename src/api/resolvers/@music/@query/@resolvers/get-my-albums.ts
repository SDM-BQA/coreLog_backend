...
export const get_my_albums = async (_parent: any, args: { filter?: AlbumFilter }, ctx: any) => {
    try {
        const user = await get_auth_user(ctx.req);
        const filter = args.filter ?? {};

        const result = await quick_list(album_model, {
            ...
            match: {
                ...
                ...(filter.artists?.length   && { artist: filter.artists }),
        ...
        console.error("Get My Albums Error:", error.message);
        return { albums: [], total_count: 0, current_page: 1, per_page: 10, page_count: 1, has_next_page: false };
    }
};
